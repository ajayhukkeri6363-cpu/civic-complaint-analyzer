...
@app.route('/api/clear_all_complaints')
def api_clear_all_complaints():
    try:
        conn = get_db_connection()
        cursor = conn.cursor()
        execute_db(cursor, "DELETE FROM votes")
        execute_db(cursor, "DELETE FROM complaints")
        try: execute_db(cursor, "DELETE FROM notifications")
        except: pass
        conn.commit()
        conn.close()
        return jsonify({'status': 'success', 'message': 'All complaints and votes erased cleanly.'})
    except Exception as e:
